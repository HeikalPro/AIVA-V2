...
async def _ensure_system_nav_permission(db: Database) -> None:
    """Grant the system-health nav key to super admin + developer (existing DBs)."""
    if "system" not in ALL_NAV_KEYS:
        return
    role_rows = await db.fetch_all("SELECT id, name FROM AIVA_roles")
    for row in role_rows:
        role_name = str(row["name"])
        if role_name not in _SYSTEM_NAV_ROLES:
            continue
        role_id = int(row["id"])
        existing = await db.fetch_one(
            """
            SELECT 1 FROM AIVA_role_nav_permissions
            WHERE role_id = :role_id AND nav_key = 'system'
            """,
            {"role_id": role_id},
        )
        if existing:
