...
async def notify_developers_new_ticket(
    *,
    ...
    return DeveloperNotifyOut(
        status="failed",
        message=(
            "Email was not sent. Sign out and sign in again with Zoho (accept Mail permissions). "
            "If it still fails, in zoho_auth/.env put quotes around ZOHO_SCOPE, then re-login. "
            "From AIVA-V2: python -m zoho_auth --force-login"
        ),
        recipients=recipients,
    )
